...
throwHttp = ETT__.t "Network.HTTP.Client.Types.throwHttp" ETT__.$ throwIO . HttpExceptionContentWrapper

toHttpException :: Request -> HttpExceptionContentWrapper -> HttpException
toHttpException req (HttpExceptionContentWrapper e) = ETT__.t "Network.HTTP.Client.Types.toHttpException" ETT__.$ HttpExceptionRequest req e
...
equalCookie a b = ETT__.t "Network.HTTP.Client.Types.equalCookie" ETT__.$ and
...
equivCookie a b = ETT__.t "Network.HTTP.Client.Types.equivCookie" ETT__.$ name_matches && domain_matches && path_matches
...
compareCookies c1 c2
    | S.length (cookie_path c1) > S.length (cookie_path c2) = ETT__.t "Network.HTTP.Client.Types.compareCookies" ETT__.$ LT
    | S.length (cookie_path c1) < S.length (cookie_path c2) = ETT__.t "Network.HTTP.Client.Types.compareCookies" ETT__.$ GT
    | cookie_creation_time c1 > cookie_creation_time c2 = ETT__.t "Network.HTTP.Client.Types.compareCookies" ETT__.$ GT
    | otherwise = ETT__.t "Network.HTTP.Client.Types.compareCookies" ETT__.$ LT

-- | See 'equalCookie'.
--
-- @since 0.7.0
equalCookieJar :: CookieJar -> CookieJar -> Bool
equalCookieJar (CJ cj1) (CJ cj2) = ETT__.t "Network.HTTP.Client.Types.equalCookieJar" ETT__.$ and $ zipWith equalCookie cj1 cj2

-- | See 'equalCookieJar', 'equalCookie'.
--
-- @since 0.7.0
equivCookieJar :: CookieJar -> CookieJar -> Bool
equivCookieJar cj1 cj2 = ETT__.t "Network.HTTP.Client.Types.equivCookieJar" ETT__.$ and $
...
simplify (RequestBodyLBS lbs) = ETT__.t "Network.HTTP.Client.Types.simplify" ETT__.$ Left (L.length lbs, fromLazyByteString lbs)
simplify (RequestBodyBS bs) = ETT__.t "Network.HTTP.Client.Types.simplify" ETT__.$ Left (fromIntegral $ S.length bs, fromByteString bs)
simplify (RequestBodyBuilder len b) = ETT__.t "Network.HTTP.Client.Types.simplify" ETT__.$ Left (len, b)
simplify (RequestBodyStream i gp) = ETT__.t "Network.HTTP.Client.Types.simplify" ETT__.$ Right (Just i, gp)
simplify (RequestBodyStreamChunked gp) = ETT__.t "Network.HTTP.Client.Types.simplify" ETT__.$ Right (Nothing, gp)
simplify (RequestBodyIO _mbody) = ETT__.t "Network.HTTP.Client.Types.simplify" ETT__.$ error "FIXME No support for Monoid on RequestBodyIO"

builderToStream :: (Int64, Builder) -> (Maybe Int64, GivesPopper ())
builderToStream (len, builder) = ETT__.t "Network.HTTP.Client.Types.builderToStream" ETT__.$
    (Just len, gp)
  where
    gp np = do
...
redactSensitiveHeader toRedact h@(name, _) = ETT__.t "Network.HTTP.Client.Types.redactSensitiveHeader" ETT__.$
