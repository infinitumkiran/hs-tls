...
generateParams
    ...
    -- ^ generator
    -> m Params
generateParams bits generator = ETT__.tm "Crypto.PubKey.DH.generateParams" ETT__.$
...
generatePrivate (Params p _ _) = ETT__.tm "Crypto.PubKey.DH.generatePrivate" ETT__.$ PrivateNumber <$> generateMax p

-- | calculate the public number from the parameters and the private key
-- this number is usually called Y in DH text.
calculatePublic :: Params -> PrivateNumber -> PublicNumber
calculatePublic (Params p g _) (PrivateNumber x) = ETT__.t "Crypto.PubKey.DH.calculatePublic" ETT__.$ PublicNumber $ expSafe g x p
...
generatePublic = ETT__.t "Crypto.PubKey.DH.generatePublic" ETT__.$ calculatePublic
...
getShared (Params p _ bits) (PrivateNumber x) (PublicNumber y) = ETT__.t "Crypto.PubKey.DH.getShared" ETT__.$ SharedKey $ i2ospOf_ ((bits + 7) `div` 8) $ expSafe y x p
